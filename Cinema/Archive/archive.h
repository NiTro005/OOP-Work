...
TArchive<T>& TArchive<T>::operator=(const TArchive<T>& other) {
    if (this != &other) { // защита от самоприсваивания
        // освобождаем память, выделенную под текущий массив данных
        delete[] _data;
        delete[] _states;

        // выделяем новый массив данных и копируем в него данные из исходного объекта
        ...
        // выделяем новый массив состояний ячеек и копируем в него данные из исходного объекта
        _states = new State[_capacity];
        for (size_t i = 0; i < _capacity; i++) {
            _states[i] = other._states[i];
        }

        // обновляем значения полей _size и _deleted
        _size = other._size;
